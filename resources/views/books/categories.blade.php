    ...
    @foreach($books as $book)
        ...
        <div class="card float-left mx-auto post" style="width: 20rem; margin: 1rem;">
            ...
            <div class="card-body text-center">
                <h4 class="card-title ">{{$book->title}}</h4><a href="{{route('books.category', $book->categories->id)}}"><button type="button" class="btn btn-outline-info">{{$book->categories->name}}</button></a>  <a href="{{ route('books.author', $book->author->id) }}"  class="btn btn-outline-secondary" style="margin: 2%;">{{$book->author->name}}</a>
                </p>
